...
<body class="page-header-fixed">
		                     ...
		                     <table class="table table-striped table-bordered table-hover" id="data-table">
		                        ...
		                        <tbody>
							        ...
							        <tr class="odd gradeX">
							            ...
							            <td>${ item.enable }</td>
							            <td>${ item.flag }</td>
							            <td>${ item.endTime.getTime().toLocaleString() }</td>
							        </tr>
							        </c:forEach>
		                        </tbody>
		                     </table>
   ...
   <script type="text/javascript">
         ...
         $(".table-toolbar").toolbarLite({
             items: [
				 { link: true, display: "新建", css: "icon-plus", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/project/add", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>" },
				 { splitter: true }, 
				 { link: true, display: "编辑", css: "icon-edit", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/project/edit/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
					  	selector: "#data-table .checkboxes", mustSelect: "请先选择数据！", singleSelect: "该操作只支持单选！"},
				 { splitter: true }, 
                 { link: true, display: "启用", css: "icon-ok", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/project/enable/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>",
                	 selector: "#data-table .checkboxes", mustSelect: "请先选择数据！", singleSelect: "该操作只支持单选！"},
                 { splitter: true }, 
                 { link: true, display: "禁用", css: "icon-remove", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/project/disable/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
                 	 selector: "#data-table .checkboxes", mustSelect: "请先选择数据！", singleSelect: "该操作只支持单选！"},
                 { splitter: true }, 
                 { link: true, display: "删除", css: "icon-trash", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/project/delete/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
